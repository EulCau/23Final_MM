...
class DLAMetricsEvaluator:
	def __init__(
		...
		x, y, z = self.dendrites_indices[-1]
		self.max_extent = self._compute_extent(x, y, z).max()
		self.bins = np.linspace(0, self.max_extent, self.num_layers + 1)
	...
	def compute_volume_ratio_and_density(self):
		...
		for x, y, z in self.dendrites_indices:
			...
			indices = np.digitize(extent, self.bins) - 1
			indices = np.clip(indices, 0, self.num_layers - 1)
